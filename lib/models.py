...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, moondexd):
        golist = moondexd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_moondexd(moondexd, item)
    ...
    @classmethod
    def import_gobject_from_moondexd(self, moondexd, rec):
        import decimal
        import moondexlib
        import inflection
        ...
        # shim/moondexd conversion
        object_hex = moondexlib.SHIM_deserialise_from_moondexd(object_hex)
        objects = moondexlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from moondexd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from moondexd, with every run
        ...
        # get/create, then sync payment amounts, etc. from moondexd - Moondexd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(moondexd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from moondexd! %s" % e)
            govobj.vote_delete(moondexd)
            return (govobj, None)
    ...
    def vote_delete(self, moondexd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(moondexd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, moondexd, signal, outcome):
        import moondexlib
        ...
        output = moondexd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = moondexlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(moondexd, signal)

    def sync_network_vote(self, moondexd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = moondexd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 moondex addr, non-multisig
            if not moondexlib.is_valid_moondex_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Moondex address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (moondexlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/MoondexDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import moondexlib
        obj_data = moondexlib.SHIM_serialise_for_moondexd(self.serialise())
    ...
    def prepare(self, moondexd):
        try:
            object_hash = moondexd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not moondexlib.is_valid_moondex_address(addr, config.network):
    ...
    def hash(self):
        import moondexlib
        return moondexlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, moondexd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - moondexd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, moondexd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - moondexd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, moondexd):
        now = int(time.time())
        return (self.created_at < (now - moondexd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, moondexd):
        if self.is_expired(moondexd):
            return False

        return True

    def is_deletable(self, moondexd):
        if self.is_expired(moondexd):
            return True
