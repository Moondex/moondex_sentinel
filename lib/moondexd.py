...
class MoondexDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some moondexd methods
    ...
    @classmethod
    def from_moondex_conf(self, moondex_dot_conf):
        from moondex_config import MoondexConfig
        config_text = MoondexConfig.slurp_config_file(moondex_dot_conf)
        creds = MoondexConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = moondexlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = moondexlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = moondexlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_moondexd_proto_version_with_sentinel_ping)
