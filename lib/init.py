...
def has_moondex_conf():
    import config
    import io

    valid_moondex_conf = False

    # ensure moondex_conf exists & readable
    #
    # if not, print a message stating that Moondex Core must be installed and
    # configured, including JSONRPC access in moondex.conf
    try:
        f = io.open(config.moondex_conf)
        valid_moondex_conf = True
    except IOError as e:
        print(e)

    return valid_moondex_conf
...
def main():
    ...
    if not has_moondex_conf():
        print("MoondexCore must be installed and configured, including JSONRPC access in moondex.conf")
        sys.exit(1)
